...
export async function getSyncedHistories(id: string, session?: ClientSession){
  try{
    const histories = await patientSyncModel.findOne({ id }).select({ id: 1, secondaries: 1 }).session(session ?? null);  
    if(histories)
      return histories;
    // buscar nas referencias passadas do utente
    const allHistory = await patientSyncModel.find().select({ id: 1, secondaries: 1 }).session(session ?? null); 
