...
export async function openPatientProcess(patientId: string, location: WorkLocation){
  try{
    ...
    if(existProcess?.isInUse && existProcess.userId?.toString() !== await getUserId()){      
      const user = await getUser(existProcess.userId?.toString() as string);
      const fullname = user?.fullname ?? "utilizador desconhecido";
      throw new Error(`Processo ocupado pelo Sr(a).${getFirstAndLastName(fullname)}!`, { cause: "busy" });
    } 
...
export async function getSyncedHistories(id: string, session?: ClientSession){
  try{
    const histories = await patientSyncModel.findOne({ id }, null, { session }).select({ id: 1, secondaries: 1 });  
    if(histories)
      return histories;
    // buscar nas referencias passadas do utente
    const allHistory = await patientSyncModel.find({}, null, { session }).select({ id: 1, secondaries: 1 }); 
